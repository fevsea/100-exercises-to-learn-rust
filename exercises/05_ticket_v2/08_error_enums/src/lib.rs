...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        ...
        Err(TicketNewError::InvalidDescription(_)) => {
            Ticket::new(title, DEFAULT_DESCRIPTION.to_string(), status).unwrap()
        },
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(InvalidTitle("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            return Err(InvalidTitle("Title cannot be longer than 50 bytes".to_string()));
        }
        if description.is_empty() {
            return Err(InvalidDescription("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(InvalidDescription("Description cannot be longer than 500 bytes".to_string()));
        }
