    ...
    T: Display + Send + Sync + 'static,
{
    ...
    tokio::join!(
        tokio::spawn(handle_listener(first, arc.clone())),
        tokio::spawn(handle_listener(second, arc.clone())),
    );
}

async fn handle_listener<T>(tcp_listener: TcpListener, reply: Arc<T>)
where
    T: 'static + Display + Send + Sync,
{
    loop {
        let (mut conn, _) = tcp_listener.accept().await.unwrap();
        let local_reply = reply.clone();
        tokio::spawn(async move {
            let (_, mut sender) = conn.split();
            sender.write_all( local_reply.to_string().as_bytes()).await.unwrap();
        });
    }
}
